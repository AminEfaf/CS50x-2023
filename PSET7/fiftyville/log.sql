...
The provided SQL code for the Fiftyville problem appears to be mostly correct. However, there are a few minor adjustments that can be made for improved readability:
...
-- The witnesses could be the accomplice. Finding the names of the witnesses from the interviews table. Also, checking their interviews' transcripts.
SELECT name, transcript
FROM interviews
WHERE year = 2021
AND month = 7
...
-- Checking how many Eugenes are present in the 'people' table.
SELECT name
FROM people
WHERE name = 'Eugene';

-- Finding the names of the 3 witnesses from the list of names of people who gave interviews on July 28, 2021. Crime report said that the witnesses mentioned "bakery" in their transcripts. Also, ordering the results alphabetically by the names of witnesses.
SELECT name, transcript
FROM interviews
WHERE year = 2021
AND month = 7
...
-- Eugene gave clues. Checking the account number of the person who withdrew money from the ATM on Leggett Street.
SELECT account_number, amount
FROM atm_transactions
WHERE year = 2021
AND month = 7
...
-- Raymond gave clues. Finding the information about the airport in Fiftyville, which would be the origin of the thief's flight.
SELECT abbreviation, full_name, city
FROM airports
WHERE city = 'Fiftyville';
...
-- Checking the list of passengers in the flight to LaGuardia Airport on July 29, 2021. Ordering the names according to their passport numbers.
SELECT passengers.flight_id, name, passengers.passport_number, passengers.seat
FROM people
...
-- Checking the phone call records to find the person who bought the tickets.
-- Checking the possible names of the caller, and ordering them according to the durations of the calls.
SELECT name, phone_calls.duration
FROM people
...
-- Checking the possible names of the call receiver, and ordering them according to the durations of the calls.
SELECT name, phone_calls.duration
FROM people
...
-- Ruth gave clues. Checking the license plates of cars within the specified time frame. Then, checking out the names of those cars' owners.
...
-- Bruce must be the thief as he is present in all the 4 lists - list of passengers, list of people who did the specific ATM transactions, list of people who called, and list of people who drove away in cars from the bakery.
-- He must have escaped to New York City, as he took the flight to LaGuardia Airport.
